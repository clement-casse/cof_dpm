...
impl PostgresRepo {
    ...
    /// # Errors
    ///
    /// The Error can be a `sqlx::MigrateError` if the migration fails.
    pub async fn new(pg_pool: PgPool) -> Result<Self, Error> {
        sqlx::migrate!("src/repo/postgres/migrations")
            .run(&pg_pool)
            .await
    ...
    ///
    /// The Error can be a `sqlx::MigrateError` if the migration fails.
    pub async fn new_from_arc(arc_pool: Arc<PgPool>) -> Result<Self, Error> {
        sqlx::migrate!("src/repo/postgres/migrations")
            .run(&*arc_pool)
            .await
            .context("Failed to run the Postgres migration when starting the repository")?;

        Ok(Self { pool: arc_pool })
    }
}
